...
export default function CustomerPortal({ access, customerAccount }) {
  ...
  const [canEditUnacknowledged, setCanEditUnacknowledged] = useState(false);
  ...
  async function signOut() {
    ...
    setLockedCustomer("");
    setCanEditUnacknowledged(false);
    setCustomerOrders([]);
  ...
  async function loadLockedCustomer() {
    ...
    try {
      ...
      const propCustomer = String(customerAccount?.company_name || "").trim();
      if (propCustomer) {
        setLockedCustomer(propCustomer);
        setCanEditUnacknowledged(Boolean(customerAccount?.can_edit_unacknowledged));
        setForm(buildEmptyForm(propCustomer));
        await loadCustomerOrders(propCustomer);
        setLoadingProfile(false);
        return;
      }
      ...
      const { data, error } = await supabase
        ...
        .select("company_name, can_edit_unacknowledged, active")
      ...
      setCanEditUnacknowledged(Boolean(data?.can_edit_unacknowledged));
  ...
  function startEditOrder(order) {
    if (order.status !== "Unacknowledged" || !canEditUnacknowledged) {
  ...
  async function saveEditOrder(e) {
    ...
    try {
      ...
      if (currentOrder.status !== "Unacknowledged" || !canEditUnacknowledged) {
        alert("This order can no longer be edited. Please call the plant directly.");
        closeEditModal();
        await loadCustomerOrders(lockedCustomer);
        return;
