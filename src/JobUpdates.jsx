...
export default function JobUpdates({ access }) {
  ...
  async function fetchJobs(showSpinner = false) {
    ...
    let company = access?.customerAccount?.company_name || "";
    // If company is not set, fetch from customer_accounts using current user
    if (!company && access?.session?.user?.id) {
      const { data: account, error: accError } = await supabase
        .from("customer_accounts")
        .select("company_name")
        .eq("auth_user_id", access.session.user.id)
        .single();
      if (!accError && account?.company_name) {
        company = String(account.company_name).trim();
      }
    }
    ...
    // Query manager_live_jobs, filter by customer_name
    const { data, error } = await supabase
      .from("manager_live_jobs")
      .select("*")
      .eq("customer_name", company)
      .order("load_date", { ascending: false })
      .order("load_time", { ascending: false });

    if (error) {
      console.error("Error loading jobs:", error, "company:", company);
      setJobs([]);
    } else {
      setJobs(data || []);
    }
  ...
  const groupedRows = useMemo(() => {
    ...
    for (const job of filteredJobs) {
      ...
      const key = `${companyName}__${orderId}__${mix}`;
      if (!grouped[key]) {
        grouped[key] = {
          key,
          order_id: orderId,
    ...
    return rows;
  }, [filteredJobs, sortBy, companyName]);
  ...
  return (
    <div style={{ padding: 20 }}>
      <h2 style={{ margin: "0 0 16px 0" }}>Job Activity (Your Company Only)</h2>
...
// (Removed duplicate/unreachable code after main export default function)
